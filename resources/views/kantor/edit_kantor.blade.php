...
<!-- Page Heading -->
<h1 class="h3 mb-4 text-gray-800">{{ $title ?? __('Edit Data Surat Kantor') }}</h1>
...
<div class="content">
    ...
    <div class="form-group">
        ...
        <!-- <div class="form-group">
            ...
            <option value="{{ $item->id }}" {{ old('users_id', $kantor->users_id) == $item->id ? 'selected' : null }}>{{ $item->role_user}}</option> 
            @endforeach
            </select>
            <div class="text-danger">
                @error('users_id')
        ...
        <label>Karyawan</label>
            ...
            <option value="{{ $item->id }}" {{ old('karyawan_id', $item->karyawan_id) == $item->id ? 'selected' : null }}>{{ $item->nama}}</option>
            @endforeach
            </select> 
            ...
            <div> 
            <a href="{{ asset('lampiran_surat/'. $kantor->lampiran_surat )}}" target="_blank" rel="noopener noreferrer"> {{$kantor->lampiran_surat}} </a>  </div>
            <div class="text-danger">
